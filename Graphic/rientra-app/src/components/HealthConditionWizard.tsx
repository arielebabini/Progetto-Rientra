...
export default function HealthConditionWizard({ workerId, currentConditions, allCoreSets, onClose, onSaved, onStepChange }: HealthConditionWizardProps) {
  ...
  const [isReviewSearchFocused, setIsReviewSearchFocused] = useState(false);
    ...
    <div className="hc-wizard-overlay">
      <div className="hc-wizard-modal">
        ...
        <div className="hc-wizard-header">
          <div>
            <h2 className="hc-wizard-title">
              {step === 'select' ? 'Choose New Codes' : 'Review & Qualifiers'}
            </h2>
            <p className="hc-wizard-subtitle">
              {step === 'select'
                ? 'Select additional codes to define the condition of the worker.'
            ...
            <>
              <div className="hc-wizard-toolbar">
                <div className="hc-search-wrapper">
                  ...
                  />
                  {search.length > 0 ? (
                    <span className="hc-search-action-icon" onMouseDown={(e) => { e.preventDefault(); setSearch(''); }}>
                      <SmallXIcon />
                    </span>
                  ) : isSearchFocused ? (
                    <span className="hc-search-action-icon">
                      <ArrowRightIcon />
                    </span>
                  ) : null}
                </div>
                  ...
                  <table className="hc-wizard-table">
                    <thead>
                      <tr>
                        ...
                        <th style={{ width: 120 }}>ICF Code</th>
                        <th style={{ width: '28%' }}>Code Name</th>
                        <th style={{ width: '20%' }}>Category</th>
                        <th style={{ width: 'auto' }}>Core Set</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredCodes.map(c => {
              ...
              <div className="hc-review-toolbar">
                ...
                <div className="hc-search-wrapper hc-search-wrapper--review">
                  ...
                  <input
                    ...
                    onFocus={() => setIsReviewSearchFocused(true)}
                    onBlur={() => setIsReviewSearchFocused(false)}
                  />
                  {reviewSearch.length > 0 ? (
                    <span className="hc-search-action-icon" onMouseDown={(e) => { e.preventDefault(); setReviewSearch(''); }}>
                      <SmallXIcon />
                    </span>
                  ) : isReviewSearchFocused ? (
                    <span className="hc-search-action-icon">
                      <ArrowRightIcon />
                    </span>
                  ) : null}
