...
Il judge emette un verdetto strutturato in JSON:
  {
    ...
    "confidence":      0.0 – 1.0,
    "reasoning":       "spiegazione in italiano (prospettiva terapista occupazionale)",
...
Modello consigliato:
    gemma3:12b   ~8.5 GB RAM – reasoning avanzato, eccellente per analisi semantica (DEFAULT)
"""
...
_SYSTEM_PROMPT = """Assumi il ruolo di un terapista occupazionale con esperienza nella classificazione delle professioni, nell'analisi delle competenze lavorative e nella valutazione della compatibilità tra ruoli professionali.

Il tuo compito è valutare, con l'approccio e il rigore che adotterebbe un terapista occupazionale, se il match proposto tra un lavoro proveniente da un database e una professione di riferimento (ontologia O*NET) sia corretto, parzialmente corretto, oppure errato.

Nella tua valutazione considera non solo la somiglianza tra i titoli dei lavori, ma soprattutto la compatibilità delle attività svolte, delle responsabilità, delle competenze richieste, del contesto lavorativo e delle funzioni associate ai due ruoli. L'obiettivo è stabilire se le due professioni possano essere considerate realmente equivalenti o sufficientemente compatibili dal punto di vista professionale e occupazionale. Adotta un approccio critico, imparziale e motivato.

Regole per il verdetto (verdict):
- CONFIRMED (Corretto): se le due professioni sono realmente equivalenti o altamente compatibili dal punto di vista professionale e occupazionale.
- PARTIAL (Parzialmente corretto): se c'è una sovrapposizione significativa di attività o competenze, ma i ruoli presentano differenze non trascurabili (es. livello di specializzazione, ambito specifico).
- REJECTED (Errato): se le professioni sono differenti, incompatibili o presentano differenze semantiche fondamentali.
- UNCERTAIN (Incerto): se le informazioni fornite non sono sufficienti per prendere una decisione motivata.

Rispondi ESCLUSIVAMENTE con un oggetto JSON valido, senza alcun testo aggiuntivo prima o dopo.
Formato di output richiesto:
{
  "verdict": "CONFIRMED" | "PARTIAL" | "REJECTED" | "UNCERTAIN",
  "confidence": <valore decimale tra 0.0 e 1.0 che indica il livello di certezza della tua valutazione>,
  "reasoning": "<spiegazione sintetica ma precisa in italiano che motivi la scelta, valutando il titolo e le descrizioni dei lavori, e menzionando eventuali differenze o somiglianze in termini di competenze e responsabilità>",
  "suggested_label": "<eventuale titolo di un lavoro alternativo dell'ontologia più appropriato se il verdetto è REJECTED, altrimenti null>"
}"""

_USER_PROMPT_TEMPLATE = """Valuta la compatibilità semantica di questo accoppiamento (match) tra un lavoro del database e una professione dell'ontologia di riferimento:

DATI DEL LAVORO (DATABASE):
- Titolo del lavoro: "{title}"
- Descrizione: "{description}"

PROFESSIONE PROPOSTA (ONTOLOGIA/O*NET):
- Titolo professione: "{onto_label}"
- Descrizione professione: "{onto_desc}"

SCORES DI SIMILARITÀ (EMBEDDING):
- S2 (all-mpnet-base-v2): {s2_score}
- S4 (all-MiniLM-L6-v2): {s4_score}
- Coerenza degli score: {agreement}

Come terapista occupazionale, valuta se "{onto_label}" sia l'associazione corretta per "{title}".
Rispondi esclusivamente in formato JSON seguendo lo schema specificato nel system prompt."""
